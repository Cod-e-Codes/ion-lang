...
- `test_closure_unused_borrow_error.ion` - Move closure while an unused `&` of that binding is still in scope (negative, `BorrowConflict`)
- `test_unwrap_after_borrow.ion` - `Box::unwrap` after the `&` carrier's last use, outside a closure (exit 1)
- `test_closure_copy_after_borrow.ion` - `Copy` capture after the `&` carrier's last use (exit 10)
...
- `test_lt_literal_lhs_with_call.ion` - `if 5 < x { f(x); }` / `<=` parse correctly (not struct-lit)

...
Built-ins: `Vec<T>`, `String`, `Box<T>`, `Option<T>`, `Result<T, E>` (define enums in-file or import). `Vec::get` / `Vec::pop` move elements out; use `Vec::get_ref(&v, i)` for read-only in-function peek (`Option<&T>`, local only). `Slice::get_ref(&s, i)` is the same for `&[]T` (and arrays via coercion). `String::get(&s, i)` returns `Option<u8>` without panicking. Match on `&Enum` from `get_ref` dispatches variants directly (no `*` deref). Struct field paths support `=` and `+=` on owned and `&mut` receivers; there is no assign-through a bound scalar `&mut int`. Nested generics such as `Vec<Vec<int>>` parse as consecutive `>` closings. String literals and `&String` coerce to `&str` at call sites; string literals also coerce to owned `String` in `let` bindings and when passed to `String` parameters.

...
- **Match-arm last-expr re-walk**: `infer_block_result_type` re-checks the last statement after `check_stmt`. Restore pattern-binding ownership and reset arm-local names to Valid, or `let row = ...; Vec::set(..., row)` reports a false UseAfterMove (`test_vec_get_putback_named.ion`). The 0.1.15 restore of `after_bindings` only covered the scrutinee / pattern names.
...
- **`Box::unwrap` unannotated let**: IR let typing must peel `Box<T>` via the same Call return-type helper as `build_expr` (not `function_returns` then default `int`). Otherwise `let n = Box::unwrap(boxed)` with `boxed: Box<Node>` emits `int n` / `int* _box` while field access still type-checks (`test_box_unwrap_struct_let.ion`).
- **Unannotated `let x = Enum::Variant`**: IR let typing must use `Expr::EnumLit` (`Type::Enum(name)`, or `Type::Generic` when tuple args match the enum's type-param count). The default `infer_type_from_expr` fallback is `int`, so `let x = Flag::On` used to emit `int x` while match still type-checked (`test_enum_unannotated_let.ion`, `test_enum_generic_unannotated_let.ion`).
...
- **String literal call args**: parameters typed `String` need `ion_string_from_literal` at the call site, not only on `let s: String = "…"` (see `test_string_call_arg_literal.ion`). Multi-file mode only registers the current module's param types; a `StringLit` arg with unknown callee params still needs the wrap (`test_multi_fmt_io.ion`).

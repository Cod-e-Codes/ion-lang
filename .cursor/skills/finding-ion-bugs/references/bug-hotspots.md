...
- **Loop unwind**: `break`/`continue` must `emit_frame_cleanup` from the loop body frame through inner `if`/`match` frames (clone; do not mark `dropped`). Always set `loop_break_label` so `break` inside `match` inside `while`/`for` gotos the loop exit, not the `switch`. `continue` uses `loop_continue_label` so `for` steps still run (`test_loop_continue_nested_if_drop.ion`, `test_for_continue_drop_step.ion`, `test_match_break_in_while.ion`).
